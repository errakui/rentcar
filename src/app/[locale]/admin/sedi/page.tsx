  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="heading-2">{t("locations")}</h1>
          <p className="text-sm text-gray-500 mt-1">{t("locationsConfigured", { count: locations.length })}</p>
        </div>
        <button onClick={() => setShowForm(!showForm)} className="btn-primary flex items-center gap-2 text-sm">
          <Plus className="w-4 h-4" /> {t("newLocation")}
        </button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="card p-6 mb-6 animate-fadeIn">
          <h3 className="text-sm font-semibold mb-4">{t("newLocation")}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="label-field">{t("locationName")} *</label>
            ...
            <div>
              <label className="label-field">{t("canton")} *</label>
            ...
            <div>
              <label className="label-field">{t("deliveryFee")} (CHF)</label>
            ...
            <div>
              <label className="label-field">{t("pickupFee")} (CHF)</label>
            ...
            <div key={loc.id} className="card p-5">
              ...
              {(loc.deliveryFee || loc.pickupFee) && (
                <div className="mt-3 pt-3 border-t border-white/[0.04] flex gap-4 text-xs text-gray-400">
                  {loc.deliveryFee && <span>{t("deliveryLabel", { amount: loc.deliveryFee })}</span>}
                  {loc.pickupFee && <span>{t("pickupLabel", { amount: loc.pickupFee })}</span>}
                </div>
              )}
            </div>
          ))}
