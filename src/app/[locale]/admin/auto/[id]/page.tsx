...
export default function EditCarPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/admin/cars/${id}/images`).then((r) => r.json()),
    ]).then(([car, locs, images]) => {
      setForm({
        brand: car.brand,
        model: car.model,
        trim: car.trim || "",
      ...
      setLocations(locs);
      setGalleryImages(
        Array.isArray(images) ? images.map((img: any) => ({ id: img.id, url: img.url })) : []
      );
      setFetching(false);
    });
  }, [id]);
    ...
    try {
      ...
      if (res.ok && data.url) {
        if (type === "cover") {
          ...
          const imgRes = await fetch(`/api/admin/cars/${id}/images`, {
            method: "POST",
          ...
          if (imgRes.ok) {
            ...
            setGalleryImages((prev) => [...prev, { id: img.id, url: img.url }]);
          }
        }
      }
    } catch (err) {
  ...
  const handleRemoveGallery = async (index: number) => {
    ...
    if (img.id) {
      await fetch(`/api/admin/cars/${id}/images`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="heading-2">{t("editCar")}</h1>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            {galleryImages.map((img, i) => (
              <div key={img.id || i} className="relative aspect-[4/3] rounded-xl overflow-hidden border border-white/10">
        ...
        <div className="flex gap-4">
          ...
          <Link href="/admin/auto" className="btn-secondary">{t("cancel")}</Link>
        </div>
      </form>
    </div>
  );
