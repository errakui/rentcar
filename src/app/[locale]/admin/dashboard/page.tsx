...
export default function DashboardPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="heading-2">{t("dashboard")}</h1>
      </div>
        ...
        {[
          {
            ...
            sub: `${stats?.totalCars ?? 0} ${t("total")}`,
          },
          {
            icon: MessageSquare,
            label: t("newLeads"),
            value: stats?.newLeads ?? "—",
            sub: `${stats?.totalLeads ?? 0} ${t("total")}`,
          },
          {
            icon: MapPin,
            label: t("locations"),
      ...
      <div className="card">
        <div className="p-5 border-b border-white/[0.06]">
          <h2 className="text-sm font-semibold">{t("recentLeads")}</h2>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  <th className="table-header">{t("totalAmount")}</th>
                ...
                {recentLeads.map((lead: any) => (
                  <tr key={lead.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="table-cell">
                      ...
                      >
                        {t(`status_${lead.status.toLowerCase()}`)}
                      </span>
                    </td>
                  </tr>
                ))}
